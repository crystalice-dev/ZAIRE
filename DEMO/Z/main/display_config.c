...
void display_main_UI(void){
    ...
    display_datetime();
    if(number_paired_peers > 1)display_walkie_paired();
    display_location();
    display_elevation();
    u8g2_SetFont(&display, u8g2_font_6x12_tr);
    display_battery(100); // percent
...
void display_datetime() {
    const char *time = "12:30";
    const char *date = "Jan-1";
...
void display_walkie_paired() {
    char el[64];
    snprintf(el, sizeof(el), "W:%d", number_paired_peers - 1);
    u8g2_DrawStr(&display, 75, 8, el);
    
}

void display_location(){
    u8g2_DrawStr(&display, 1, 18, "T-Speed: 16mph");
    u8g2_DrawStr(&display, 1, 28, "Dst: 10mi");
}

void display_elevation(){
    char el[260];
    if(1){
        snprintf(el, sizeof(el), "Stops:%d", 3); //number of stops
    }else{
        snprintf(el, sizeof(el), "%dFT", 100);
    }
    u8g2_DrawStr(&display, 80, 28, el);
}
...
void display_show_walkie_pairing(void){
    ...
    u8g2_ClearBuffer(&display);
    u8g2_SetFont(&display, u8g2_font_ncenR18_tr);
    u8g2_DrawStr(&display, 1, 30, "WALKIE");
    u8g2_SendBuffer(&display);
    vTaskDelay(pdMS_TO_TICKS(7000));
    display_target = DISPLAY_EN_OFF;
    vTaskDelay(pdMS_TO_TICKS(1000));
