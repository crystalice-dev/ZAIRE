    ...
    case NONE:
        if(data_len > 100){
            ...
            if(is_walkie_mute == false){
                if (!speakOut || data_len > 128) {
                    return;  // Ignore if not in speakOut mode or len exceeds buffer size
                }
                recOver = true;
                for (int i = 0; i < data_len; i++) {
                    receive_16bit[i] = (data[i] - 128) << 5;
                    output_16bit[i * 2] = receive_16bit[i];
                    output_16bit[i * 2 + 1] = receive_16bit[i];
                }
                I2Swrite(output_16bit, data_len * 2);  // Double count for stereo channels
            }
