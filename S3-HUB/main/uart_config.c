...
esp_err_t uart_init(void){
    ...
    err0 = uart_driver_install(BK_UART_NUM, BUF_SIZE * 2, 0, 0, NULL, 0);
    err1 = uart_param_config(BK_UART_NUM, &bk_uart_config); 
    err2 = uart_set_pin(BK_UART_NUM, BK_TX, BK_RX, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
...
void bk_uart_snd(const char* str){
    uart_write_bytes(BK_UART_NUM, str, strlen(str));
